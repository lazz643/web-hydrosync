...
const InvoicePembelian = forwardRef((props, ref) => {
    ...
    const fetchUser = useCallback(async () => {
        if (localPayment) {
            try {
                ...
                if (result.data.users.token === null) {
                    fetchUser(); // Panggil kembali fetchUser jika token null (perhatikan potensi infinite loop)
                } else {
                    setLoading(false);
                }
            } catch (err) {
    ...
    useEffect(() => {
        if (userData.token) {
            setLoading(false); // Akhiri loading jika token tersedia
        }
    }, [userData.token]);
        ...
        <div className={styles.container} ref={componentRef}>
            <>
                ...
                <Link href="/UserDashboard">
                    <button type='button' className={`${styles.button} ${styles.hideOnPrint}`}>Kembali ke Dashboard</button>
                </Link>
...
InvoicePembelian.displayName = "InvoicePembelian";
